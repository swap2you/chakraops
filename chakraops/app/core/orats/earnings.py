...
"""
R24.5: ORATS earnings advisory — request-time fields from /datav2/cores.
Used for hero pill and risk flags only; never blocks eligibility.
Earnings data is stored in eval snapshot for deterministic per-symbol recompute.
"""
...
def fetch_earnings_advisory(
    ...
    if implied_move is not None:
        try:
            result["implied_earnings_move_pct"] = float(implied_move) * 100.0
        except (TypeError, ValueError):
            pass

    # earnings_days: calendar days from as_of (America/New_York) to next_ern
    as_of_dt = as_of_utc or datetime.now(timezone.utc)
    as_of_date_str = _as_of_date_ny(as_of_dt)
    if as_of_date_str and next_ern:
        days = _calendar_days(as_of_date_str, next_ern)
        if days is not None:
            result["earnings_days"] = days
    elif isinstance(days_to_next_ern, int):
    ...
    # Status: OK if we have next date or usable data; Stale if quote > 1 day old
    if result["earnings_next_date"] is not None or result["earnings_days"] is not None:
