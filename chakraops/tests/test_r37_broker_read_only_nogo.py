...
"""R37 Robinhood read-only NO-GO — policy, status route, no unofficial clients."""
...
def test_robinhood_integration_status_is_nogo():
    ...
    for path in app_root.rglob("*"):
        ...
        for marker in FORBIDDEN_IMPORT_MARKERS:
            if marker.lower() in lower:
                # Allow documentation strings that explicitly forbid the marker in this policy module.
                rel = path.relative_to(app_root).as_posix()
                if rel in {
                    "core/broker/read_only_policy.py",
                    "core/broker/__init__.py",
                } and ("forbidden" in lower or "no-go" in lower or "nogo" in lower):
                    continue
                hits.append(f"{rel}: {marker}")
    assert hits == [], f"Forbidden Robinhood client markers under app/: {hits}"


def test_broker_status_endpoint_returns_nogo():
