...
class TestCSPGenerator:
    ...
    def test_exclusion_no_options(self) -> None:
        ...
        csp_cfg = CSPConfig(otm_pct_min=0.03, otm_pct_max=0.10)
    ...
    def test_delta_based_selection(self) -> None:
        ...
        csp_cfg = CSPConfig(
            otm_pct_min=0.03,
            otm_pct_max=0.10,
            delta_min=0.20,  # Matches strikes with abs(delta) >= 0.20
            delta_max=0.35,  # Matches strikes with abs(delta) <= 0.35
        )
