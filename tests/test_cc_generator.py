...
class TestCCGenerator:
    ...
    def test_exclusion_no_options(self) -> None:
        ...
        cc_cfg = CCConfig(otm_pct_min=0.02, otm_pct_max=0.10)
    ...
    def test_exclusion_no_strikes_in_otm_range(self) -> None:
        ...
        cc_cfg = CCConfig(
            otm_pct_min=0.15,  # 15% OTM (none of our strikes are this OTM)
            otm_pct_max=0.20,  # 20% OTM
        )
        ...
        assert len(candidates) == 0
        assert len(exclusions) >= 1
        assert any(ex.code == "NO_STRIKES_IN_OTM_RANGE" for ex in exclusions)
    ...
    def test_delta_based_selection(self) -> None:
        ...
        cc_cfg = CCConfig(
            otm_pct_min=0.02,
            otm_pct_max=0.10,
            delta_min=0.20,  # Matches strikes with delta >= 0.20
            delta_max=0.35,  # Matches strikes with delta <= 0.35
        )
    ...
    def test_otm_based_selection(self) -> None:
        ...
        # Verify selection_path is OTM_PCT
        for candidate in candidates:
            selection_path_items = [e for e in candidate.explanation if e.code == "SELECTION_PATH"]
            assert len(selection_path_items) == 1
            assert selection_path_items[0].data["selection_path"] == "OTM_PCT"
    ...
    def test_otm_only_strikes_above_spot(self) -> None:
        ...
        cc_cfg = CCConfig(
            otm_pct_min=0.02,  # 2% OTM (strike > 153)
            otm_pct_max=0.10,  # 10% OTM (strike <= 165)
        )
        ...
        for candidate in candidates:
            ...
            # Verify OTM percentage is positive
            otm_items = [e for e in candidate.explanation if e.code == "OTM_PCT"]
            if otm_items:
                otm_pct = otm_items[0].data["otm_pct"]
                assert otm_pct > 0
