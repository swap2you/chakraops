...
## R22.7 Fix Pack — Hygiene 1.2, Snapshot determinism, MTF truth

### Decision artifact hygiene 1.2
- **No prose anywhere in persisted artifacts.** Explicit list of fields that are now code-only (never prose):
  - `score_breakdown.score_caps.applied_caps[].reason` → **removed**; only `reason_code` (e.g. `REGIME_CAP`) and numeric `before`/`after`/`cap_value` persisted.
  - `diagnostics_by_symbol.*.rank_reasons.reasons` (prose strings) → **replaced** by `rank_reason_codes` (e.g. `REGIME_NEUTRAL`, `HIGH_DATA_COMPLETENESS`, `CAPITAL_EFFICIENT`).
  - `diagnostics_by_symbol.*.exit_plan.reason` → **replaced** by `exit_plan.reason_code` (e.g. `EXITPLAN_MISSING_INPUTS`, `EXITPLAN_ERROR`).
  - `earnings_by_symbol.*.note` → **replaced** by `status_code` (e.g. `EARNINGS_NOT_EVALUATED`, `EARNINGS_BLOCKED`, `EARNINGS_OK`).
- **Hygiene tests strengthened:** Full walk of persisted JSON: no string value may contain spaces (zero-whitelist); no substrings "caps score", "Not evaluated", "Exit plan not computed", "High data completeness"; no FAIL_*/WARN_*; applied_caps must have `reason_code` not `reason`. Test that uses store writer to produce artifact then validates on-disk.

### Determinism snapshot semantics
- **Snapshot rule:** When `/api/ui/eval/run` completes, `out/eval_snapshot.json` is written with `snapshot_id` (run_id), `pipeline_timestamp`, `created_at`, `quote_as_of`, `candles_as_of`, `orats_as_of`.
- **Recompute:** Default (force=false) when snapshot is fresh (< N minutes, configurable `EVAL_SNAPSHOT_VALID_MINUTES`, default 10): **uses snapshot** (no re-run); returns existing symbol row with `updated: false`, `snapshot_used: true`. With **force=true**: always re-runs with fresh data.
- **As-of / Inputs:** Symbol diagnostics response includes `snapshot_id`, `snapshot_created_at` when snapshot exists.
- **Determinism test:** `test_eval_run_writes_snapshot_recompute_without_force_uses_it` — eval/run writes snapshot; recompute without force uses same result (no drift).

### MTF no-fallback behavior
- **No copying daily into weekly/monthly.** If weekly/monthly cannot be computed (insufficient resampled bars), return `status_code=INSUFFICIENT_HISTORY` and levels null; do **not** copy daily levels into weekly/monthly rows.
- **Per-timeframe:** `bar_count`, `as_of`, `status_code` exposed; UI shows "Insufficient history" for INSUFFICIENT_HISTORY.
- **Backend unit test:** `tests/test_mtf_resampling_r227.py` — synthetic daily → weekly/monthly bar counts differ; S/R on weekly uses weekly series length.
