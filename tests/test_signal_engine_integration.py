...
class TestSignalEngineIntegration:
    ...
    def test_signal_engine_golden_json(self) -> None:
        ...
        if golden_file.exists():
            ...
            # Compare (excluding as_of which varies)
            assert actual_dict["universe_id_or_hash"] == expected_dict["universe_id_or_hash"]
            assert actual_dict["configs"] == expected_dict["configs"]
            assert actual_dict["stats"] == expected_dict["stats"]

            # Compare candidates (order matters)
            assert len(actual_dict["candidates"]) == len(expected_dict["candidates"])
            for actual_c, expected_c in zip(actual_dict["candidates"], expected_dict["candidates"]):
                assert actual_c == expected_c

            # Compare exclusions (order may vary, so compare sets)
            actual_exclusions = {json.dumps(ex, sort_keys=True) for ex in actual_dict["exclusions"]}
            expected_exclusions = {json.dumps(ex, sort_keys=True) for ex in expected_dict["exclusions"]}
    ...
    def test_signal_engine_sorting(self) -> None:
        ...
        csp_config = CSPConfig(otm_pct_min=0.03, otm_pct_max=0.10)
